...
export default class OwyOffcanvas extends OffCanvas {
    ...
    static open(url = false, data = false, callback = null, position = 'left', closable = true, delay = OffCanvas.REMOVE_OFF_CANVAS_DELAY(), fullwidth = false, cssClass = '') {
        ...
        // OffCanvasInstance.OFF_CANVAS_OPEN_CLASS = 'is-close';
    ...
    static setContent(url, data, callback, closable, delay) {
        ...
        if (data) {
            xhr = client.post(url, data, OwyOffcanvas.executeCallback.bind(this,cb));
        } else {
            xhr = client.get(url, OwyOffcanvas.executeCallback.bind(this,cb));
        }
    }
