...
class SwitchListingServiceDecorator extends ProductListingCmsElementResolver
{
    ...
    public function enrich(CmsSlotEntity $slot, ResolverContext $resolverContext, ElementDataCollection $result): void
    {
        ...
        $slot->setData($data);
        if ($query != null && (strpos($query, 'boxLayout') !== false)) {
            $flag = false;
            $value = "";
            ...
            if (!$flag) {
                ...
                $config['boxLayout']['value'] = $value;

                $translated['config']['boxLayout']['value'] = $value;

                $slot->setTranslated($translated);
                $slot->setConfig($config);
                $session->set('boxLayout', $value);
            }
        } else if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
            $config = $slot->getTranslation('config');
            
            if ($session->get('boxLayout') != null) {
                $config['boxLayout']['value'] = $session->get('boxLayout');

                $translated['config']['boxLayout']['value'] = $session->get('boxLayout');

                $slot->setTranslated($translated);
                $slot->setConfig($config);
                $session->set('boxLayout', $session->get('boxLayout'));
            } else {
                $session->set('boxLayout', $config['boxLayout']['value']);
            }
        } else {
            $session->remove('boxLayout');
        }
