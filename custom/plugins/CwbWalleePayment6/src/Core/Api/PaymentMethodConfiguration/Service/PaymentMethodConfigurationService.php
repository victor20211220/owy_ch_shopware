...
namespace CwbWalleePayment6\Core\Api\PaymentMethodConfiguration\Service;
...
use CwbWalleePayment6\CwbWalleePayment6;


/**
 * Class PaymentMethodConfigurationService
 *
 * @package CwbWalleePayment6\Core\Api\PaymentMethodConfiguration\Service
 */
class PaymentMethodConfigurationService {
	...
	/**
	 * @var \CwbWalleePayment6\Core\Util\LocaleCodeProvider
	 */
	 ...
	 *
	 * @return \CwbWalleePayment6\Core\Api\PaymentMethodConfiguration\Service\PaymentMethodConfigurationService
	 */
	...
	private function disablePaymentMethodConfigurations(Context $context): void
	{
		...
		if (!empty($paymentMethodConfigurationEntities)) {
			...
			/**
			 * @var $paymentMethodConfigurationEntity \CwbWalleePayment6\Core\Api\PaymentMethodConfiguration\Entity\PaymentMethodConfigurationEntity
			 */
		...
		try {
			$query = "UPDATE payment_method 
				  	  ...
				  	  WHERE handler_identifier=:handler_identifier AND id NOT IN (
				  	  	SELECT payment_method_id FROM cwb_wallee_payment6_method_configuration
				  	  )";

			$params = [
				'handler_identifier' => CwbWalleePayment6Handler::class,
			];
	 ...
	 * @param \Shopware\Core\Framework\Context $context
	 *
	 * @return \CwbWalleePayment6\Core\Api\PaymentMethodConfiguration\Entity\PaymentMethodConfigurationEntity|null
	 */
	...
	): void
	{
		...
		$pluginId         = $pluginIdProvider->getPluginIdByBaseClass(
			CwbWalleePayment6::class,
			$context
		);

		$data = [
			'id'                 => $id,
			'handlerIdentifier'  => CwbWalleePayment6Handler::class,
			'pluginId'           => $pluginId,
