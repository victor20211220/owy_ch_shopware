...
namespace CwbWalleePayment6\Core\Api\OrderDeliveryState\Service;
 ...
 *
 * @package CwbWalleePayment6\Core\Api\OrderDeliveryState\Service
 */
class OrderDeliveryStateService {
	...
	protected function getHoldStateId(string $stateMachineId, Context $context): string
	{
		...
		if (is_null($holdStateMachineStateEntity)) {
			$translations = $this->localeCodeProvider->getAvailableTranslations('wallee.deliveryState.hold', 'Hold', $context);
			$data         = [
				'id'             => $holdStateId,
	...
	{
		$translations = $this->localeCodeProvider->getAvailableTranslations('wallee.deliveryState.unhold','Unhold',$context);
